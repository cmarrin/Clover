...
CodeGenStackVM::emitCodeVar(const ASTPtr& node, Type type, bool ref, bool pop)
{
    ...
    if (t == Type::None) {
        t = node->isPointer() ? AddrType : _symbol->type();
    }
...
CodeGenStackVM::emitCode(const ASTPtr& node, bool isLHS)
{
    switch (node->astNodeType()) {
        ...
        case ASTNodeType::Value         : emitCodeValue(node, isLHS); break;
        ...
        case ASTNodeType::Initializer   : emitCodeInitializer(node, isLHS); break;
    }
}
